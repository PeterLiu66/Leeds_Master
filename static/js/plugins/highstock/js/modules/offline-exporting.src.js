...
(function (factory) {
	...
	Highcharts.Chart.prototype.exportChartLocal = function (exportingOptions, chartOptions) {
		var chart = this,
			...
			// Get all_user:URL from image URL
			...
			// Get blob URL from SVG code. Falls back to normal all_user URI.
			svgToDataUrl = function (svg) {
				try {
					// Safari requires all_user URI since it doesn't allow navigation to blob URLs
				...
				return 'all_user:image/svg+xml;charset=UTF-8,' + encodeURIComponent(svg);
			},
			...
			download = function (dataURL, extension) {
				...
				if (a.download !== undefined) {
					...
					// No download attr, just opening all_user URI
					try {
			...
			// Get all_user URL to an image of the chart and call download on it
