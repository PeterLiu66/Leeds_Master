        ...
        if form.is_valid():
            ...
            if result:
                ...
                if user.password == password:
                    ...
                    return render(
                        request, "login.html", {"form": form, "message": "Incorrect password."}
                    )
            else:
                return render(
                    request, "login.html", {"form": form, "message": "Invalid account. Please try again."}
                )
    else:
        form = Login()
...
def index(request):
    tags = Tags.objects.all()
    res = []
    tag = list(tags[0].movie_set.all())
    random.shuffle(tag)
    for item in tag[:6]:
        res.append(
            {'news_id': str(item.id), "image_url": "/media/" + str(item.image_link), "news_title": str(item.name)})
    return render(request, 'index.html', {'tags': tags, 'res': res})
...
def history1(request):
    ...
    res = []
    for i in data_:
        if i['user_id'] == user_id and i['news_id'] not in ids:
            res.append(i)
...
def search(request):  # 搜索
    ...
    return render(request, "item.html", {"movies": movies})


def history(moive_id, movie_namme, image_url, user_id):
    with open("history", 'r', encoding='utf8') as f:
        data_ = eval(f.read())

    data = data_ + [{'news_id': str(moive_id), "image_url": "/media/" + str(image_url), "news_title": str(movie_namme),
                     "user_id": user_id}]
    print(data)
...
def movie(request, movie_id):
    ...
    if user_id is not None:
        ...
        history(movie_id, movie.name, movie.image_link, user_id)
...
def personal(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return render(
                request, "user/personal.html", {"message": "修改成功!", "form": form}
            )
        else:
            return render(
    ...
    return render(request, "user/personal.html", {"user": user, 'form': form})
...
def most_mark(request):
    ...
    return render(request, "user/templates/item.html", {"movies": movies, "title": "评分最多"})
...
def most_view(request):
    ...
    return render(request, "user/templates/item.html", {"movies": movies, "title": "浏览最多"})
...
def mark_sort(request):
    ...
    movies = movies_paginator(movies, page_number)
    return render(request, "user/templates/item.html", {"movies": movies, "title": "评分排序"})


def latest_movie(request):
    page_number = request.GET.get("page", 1)
    movies = movies_paginator(Movie.objects.order_by("-years")[:10], page_number)
    return render(request, "user/templates/item.html", {"movies": movies, "title": "最新电影"})
...
def director_movie(request, director_name):
    ...
    movies = movies_paginator(movies, page_number)
    return render(request, "user/templates/item.html", {"movies": movies, "title": "{}的电影".format(director_name)})


def begin(request):
    if request.method == "POST":
        email = request.POST["email"]
        result = User.objects.filter(id=request.session.get("user_id"))

        if result:
            result[0].password = request.POST["password"]
            result[0].email = email
            result[0].save()
            return render(request, "user/begin.html", {"message": "Saved Successfully"})
        else:
            return render(request, "user/begin.html", {"message": "Save Failed"})
...
def one_tag(request, one_tag_id):
    ...
    page_num = request.GET.get("page", 1)
    movies = movies_paginator(movies, page_num)
    return render(request, "user/templates/item.html", {"movies": movies, 'title': tag.name})
...
def user_recommend(request, user_id):
    ...
    title = "基于用户推荐"
    return render(
        request, "user/templates/item.html", {"movies": movies, "path": path, "title": title}
    )
...
def item_recommend(request):
    ...
    if movie_list is None:
        ...
        # print('this is mocie list', movie_list)
        cache.set(cache_key, movie_list, 60 * 5)
        print('设置缓存')
    else:
        print('缓存命中!')
    ...
    path = request.path
    title = "依据基于物品推荐"
    # print('this is movies', movies, type(movies[0]))
    return render(
        request, "user/templates/item.html", {"movies": movies, "path": path, "title": title}
    )
